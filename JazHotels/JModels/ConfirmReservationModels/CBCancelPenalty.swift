...
class CBCancelPenalty : NSObject, NSCoding{

	var amountPercent : CBAmountPercent!
	var deadline : CBDeadline!
	var penaltyDescription : CBPenaltyDescription!
	var policyCode : String!
	var cancelPenalty : CBCancelPenalty!
	...
	init(fromDictionary dictionary: [String:Any]){
		if let amountPercentData = dictionary["AmountPercent"] as? [String:Any]{
			amountPercent = CBAmountPercent(fromDictionary: amountPercentData)
		}
		if let deadlineData = dictionary["Deadline"] as? [String:Any]{
			deadline = CBDeadline(fromDictionary: deadlineData)
		}
		if let penaltyDescriptionData = dictionary["PenaltyDescription"] as? [String:Any]{
			penaltyDescription = CBPenaltyDescription(fromDictionary: penaltyDescriptionData)
		}
		policyCode = dictionary["_PolicyCode"] as? String
		if let cancelPenaltyData = dictionary["CancelPenalty"] as? [String:Any]{
			cancelPenalty = CBCancelPenalty(fromDictionary: cancelPenaltyData)
		}
	}
    ...
    */
	...
	{
         amountPercent = aDecoder.decodeObject(forKey: "AmountPercent") as? CBAmountPercent
         deadline = aDecoder.decodeObject(forKey: "Deadline") as? CBDeadline
         penaltyDescription = aDecoder.decodeObject(forKey: "PenaltyDescription") as? CBPenaltyDescription
         policyCode = aDecoder.decodeObject(forKey: "_PolicyCode") as? String
         cancelPenalty = aDecoder.decodeObject(forKey: "CancelPenalty") as? CBCancelPenalty
